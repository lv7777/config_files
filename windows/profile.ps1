...
function psconf {
    code.cmd "C:\Users\$env:UserName\Documents\WindowsPowerShell\profile.ps1"
}
...
function work {
    cd ~\work\connect\ #適宜書き換え
    vagrant.exe up
    start powershell -ArgumentList "-c cd web;grunt;"
    start powershell -ArgumentList "-c cd web;gulp compass_web compass_admin default;"
    vagrant.exe ssh
}
...
function prompt {
   # our theme
	...
	write-host (shorten-path (pwd).Path) -n -f $cloc
	write-host ']' -n -f $cdelim
	Write-Host " (」・ω・)」うー！(／・ω・)／にゃー！" -ForegroundColor "Green"
	...
	write-host (shorten-path (pwd).Path) -n -f $cloc
	write-host ']' -n -f $cdelim
	Write-Host " 人生＼(^o^)／オワタ" -ForegroundColor "Red"
	if( (Get-Acl . | Select-Object AccessToString) -match (Get-Content Env:\USERNAME) ){
		return "[O] -> "
	}else{
		return "[X] -> "
